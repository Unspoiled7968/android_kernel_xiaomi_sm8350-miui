...
/*
 * The QTI common ICE library in this tree is the "private data" flavour: the
 * ICE/HWKM mmio bases are captured once by crypto_qti_init_crypto() (called
 * from ufs_qcom_ice_init()) and handed back on every subsequent call, instead
 * of being passed per-call in a struct ice_mmio_data.
 */
static void *ufs_qcom_ice_priv(struct ufs_hba *hba)
{
	struct ufs_qcom_host *host = ufshcd_get_variant(hba);

	return host->ice_priv;
}
					     ...
					     unsigned int slot)
{
	...
	err = crypto_qti_keyslot_program(ufs_qcom_ice_priv(hba), key, slot,
					   ...
					   unsigned int slot)
{
	...
	err = crypto_qti_keyslot_evict(ufs_qcom_ice_priv(hba), slot);
	if (err) {
		pr_err("%s: failed with error %d\n", __func__, err);
	}
					       ...
					       unsigned int secret_size)
{
	...
	err =  crypto_qti_derive_raw_secret(ufs_qcom_ice_priv(hba),
