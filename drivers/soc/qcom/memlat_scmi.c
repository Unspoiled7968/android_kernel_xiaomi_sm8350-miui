...
extern void rimps_memlat_init(const struct scmi_memlat_vendor_ops *ops,
...
static int scmi_memlat_probe(struct scmi_device *sdev)
{
	...
	ops = handle->devm_get_protocol(sdev, SCMI_PROTOCOL_MEMLAT, &ph);
	if (IS_ERR(ops))
		return PTR_ERR(ops);

	rimps_memlat_init(ops, ph);
	return 0;
}
