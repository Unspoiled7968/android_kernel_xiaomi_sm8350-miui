...
EXPORT_SYMBOL_GPL(iommu_iova_to_phys_hard);
...
EXPORT_SYMBOL_GPL(iommu_iova_to_pte);
...
EXPORT_SYMBOL_GPL(iommu_is_iova_coherent);
		    ...
		    struct scatterlist *sg, unsigned int nents, int prot)
{
	...
	if (ops->map_sg)
		return ops->map_sg(domain, iova, sg, nents, prot);
