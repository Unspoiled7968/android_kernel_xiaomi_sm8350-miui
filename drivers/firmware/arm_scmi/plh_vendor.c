...
static int scmi_plh_scroll_start_cmd(const struct scmi_protocol_handle *ph,
			u16 fps)
{
	return scmi_send_start_stop(ph, fps, PERF_LOCK_SCROLL_START_MSG_ID);
}

static int scmi_plh_scroll_stop_cmd(const struct scmi_protocol_handle *ph)
{
	return scmi_send_start_stop(ph, 0, PERF_LOCK_SCROLL_STOP_MSG_ID);
}
			...
			u16 val)
{
	...
	ret = ph->xops->xfer_get_init(ph, PERF_LOCK_SET_LOG_LEVEL, sizeof(*msg),
				sizeof(uint32_t), &t);
	if (ret)
		return ret;
...
static const struct scmi_protocol scmi_plh = {
	.id = SCMI_PROTOCOL_PLH,
	.owner = THIS_MODULE,
	.init_instance = &scmi_plh_vendor_protocol_init,
	.ops = &plh_proto_ops,
};
module_scmi_protocol(scmi_plh);
