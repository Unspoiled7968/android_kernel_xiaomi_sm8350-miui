	...
	void                                    *evt_payload_priv)
{
	...
	if (status_0) {
		...
		CAM_INFO(CAM_ISP,
			"ERROR time %lld:%lld",
			camif_lite_priv->error_ts.tv_sec,
			(long long)(camif_lite_priv->error_ts.tv_nsec / NSEC_PER_USEC));

		if (camif_lite_node->rdi_only_ctx)
			CAM_INFO(CAM_ISP,
				"SOF %lld:%lld EPOCH %lld:%lld EOF %lld:%lld",
				camif_lite_priv->sof_ts.tv_sec,
				(long long)(camif_lite_priv->sof_ts.tv_nsec / NSEC_PER_USEC),
				camif_lite_priv->epoch_ts.tv_sec,
				(long long)(camif_lite_priv->epoch_ts.tv_nsec / NSEC_PER_USEC),
				camif_lite_priv->eof_ts.tv_sec,
				(long long)(camif_lite_priv->eof_ts.tv_nsec / NSEC_PER_USEC));
