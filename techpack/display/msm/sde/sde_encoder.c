		...
		bool config_changed)
{
	...
	drm_bridge = drm_bridge_chain_get_first_bridge(drm_enc);
	if (sde_enc->disp_info.intf_type == DRM_MODE_CONNECTOR_DSI && drm_bridge) {
		c_bridge = container_of(drm_bridge, struct dsi_bridge, base);
		if (c_bridge) {
