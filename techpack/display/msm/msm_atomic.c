...
msm_disable_outputs(struct drm_device *dev, struct drm_atomic_state *old_state)
{
	...
	for_each_old_connector_in_state(old_state, connector,
		 ...
		 */
		drm_bridge_chain_disable(drm_bridge_chain_get_first_bridge(encoder));
		...
		drm_bridge_chain_post_disable(drm_bridge_chain_get_first_bridge(encoder));
	}
...
msm_crtc_set_mode(struct drm_device *dev, struct drm_atomic_state *old_state)
{
	...
	for_each_old_connector_in_state(old_state, connector,
		...
		drm_bridge_chain_mode_set(drm_bridge_chain_get_first_bridge(encoder),
				  mode, adjusted_mode);
		...
		struct drm_atomic_state *old_state)
{
	...
	for_each_new_connector_in_state(old_state, connector,
		 ...
		 */
		drm_bridge_chain_pre_enable(drm_bridge_chain_get_first_bridge(encoder));
		++bridge_enable_count;
	...
	for_each_new_connector_in_state(old_state, connector,
		...
		drm_bridge_chain_enable(drm_bridge_chain_get_first_bridge(encoder));
	}
	SDE_ATRACE_END("msm_enable");
}
		...
		struct drm_atomic_state *state, struct msm_commit *commit)
{
	...
	if (ret) {
		if (ret == -EINVAL)
			...
			/*
			 * 5.10 only keeps DRM_DEBUG_KMS_RATELIMITED();
			 * DRM_DEBUG_DRIVER_RATELIMITED() no longer exists.
			 */
			DRM_DEBUG_KMS_RATELIMITED("empty crtc state\n");
