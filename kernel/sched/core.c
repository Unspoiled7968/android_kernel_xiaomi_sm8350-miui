...
static void nohz_csd_func(void *info)
{
	...
	if (rq->idle_balance && !cpu_isolated(cpu)) {
				   ...
				   struct task_struct *p, int new_cpu)
{
	...
	/*
	 * WALT's fixup_busy_time() (called from set_task_cpu()) requires
	 * both runqueue locks to be held.
	 */
	rq_unpin_lock(rq, rf);
	...
	raw_spin_unlock(&cpu_rq(new_cpu)->lock);
	rq_repin_lock(rq, rf);
...
static inline bool ttwu_queue_cond(int cpu, int wake_flags)
{
	...
	/* WALT prefers remote queueing when wakeups are very frequent */
	if (walt_want_remote_wakeup() && cpu != smp_processor_id())
		return true;
...
out:
	...
	if (success && sched_predl) {
		struct rq *trq = cpu_rq(task_cpu(p));

		raw_spin_lock_irqsave(&trq->lock, flags);
		if (do_pl_notif(trq))
			cpufreq_update_util(trq,
						SCHED_CPUFREQ_WALT |
						SCHED_CPUFREQ_PL);
		raw_spin_unlock_irqrestore(&trq->lock, flags);
	}
#endif
			  ...
			  bool migrate_pinned_tasks)
{
	...
	for (;;) {
		 ...
		 */
		if (!migrate_pinned_tasks && is_per_cpu_kthread(next)) {
		...
		if (task_rq(next) != rq || !task_on_rq_queued(next)) {
			/*
			 * In the !migrate_pinned_tasks case, there is a hole
			 * between rq_unlock() and rq_relock(), where another
			 * CPU might not observe an up to date cpu_active_mask
			 * and try to move tasks around.
			 */
