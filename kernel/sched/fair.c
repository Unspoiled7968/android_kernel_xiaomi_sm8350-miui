...
redo:
	while (!list_empty(tasks)) {
		...
		case migrate_load:
			...
			/*
			 * And load based checks are skipped for prefer_spread
			 * in finding busiest group, ignore the task's h_load.
			 */
...
struct sg_lb_stats {
	...
	unsigned long load_per_task; /* Avg load per task in the group (WALT) */
	unsigned int group_no_capacity; /* Group is lacking capacity (WALT) */
...
static struct sched_group *find_busiest_group(struct lb_env *env)
{
	 ...
	 * task.
	 */
	if (env->idle != CPU_NOT_IDLE && env->prefer_spread &&
