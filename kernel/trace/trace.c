...
static void trace_check_size(struct trace_iterator iter, int cpu)
{
	...
	if (!minidump_ftrace_dump)
		return;
	buffer_size = ring_buffer_size(iter.tr->array_buffer.buffer, cpu);
...
void ftrace_dump(enum ftrace_dump_mode oops_dump_mode)
{
	...
	if (ftrace_check || trace_skip_ftrace_dump())
		goto out_enable;
	...
	while (!trace_empty(&iter)) {
		...
		if (ftrace_check && !IS_ENABLED(CONFIG_QCOM_MINIDUMP_FTRACE))
			iter.iter_flags |= TRACE_FILE_LAT_FMT;
