...
static int soc_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		for_each_rtd_codec_dais(rtd, i, codec_dai) {
			if (snd_soc_dai_stream_active(codec_dai,
				...
				struct snd_pcm_hw_params *params)
{
	...
	/* perform any hw_params fixups for hostless links */
	if ((rtd->dai_link->no_host_mode == SND_SOC_DAI_LINK_NO_HOST) &&
		...
		struct snd_soc_dapm_widget *widget, int stream)
{
	...
	for_each_card_rtds(card, be) {
		...
		for_each_rtd_dais(be, i, dai) {
			...
			/*
			 * QC: for CPU DAI widgets also compare the dai
			 * link stream name to find the right BE
			 */
			if (i < be->num_cpus && w->sname &&
			    be->dai_link->stream_name &&
			    strcmp(be->dai_link->stream_name, w->sname))
				continue;
...
int soc_new_pcm(struct snd_soc_pcm_runtime *rtd, int num)
{
	...
	if (rtd->dai_link->no_host_mode) {
		if (pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream) {
			...
			pstr->substream->hw_no_buffer = 1;
			snd_soc_set_runtime_hwparams(pstr->substream,
						     &no_host_hardware);
		}
