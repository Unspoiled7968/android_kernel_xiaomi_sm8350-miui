...
static int qcom_smd_qrtr_probe(struct rpmsg_device *rpdev)
{
	...
	/* "qcom,no-wake-svc" is the newer spelling of "qcom,non-wake-svc";
	 * accept both so that either devicetree vintage keeps working.
	 */
	prop = "qcom,no-wake-svc";
	size = of_property_count_u32_elems(rpdev->dev.of_node, prop);
	if (size <= 0) {
		prop = "qcom,non-wake-svc";
		size = of_property_count_u32_elems(rpdev->dev.of_node, prop);
	}
	...
	if (size > 0) {
		...
		rc = of_property_read_u32_array(rpdev->dev.of_node, prop,
						svc_arr.arr, size);
		if (rc) {
			kfree(svc_arr.arr);
			return rc;
